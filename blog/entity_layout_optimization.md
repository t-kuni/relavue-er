...
前段の d3-force は各連結成分の**内部**配置を決めますが、成分**同士**をどこに置くかは決めません。リレーションで繋がっていない成分間にはエッジがないため、d3-force の引力・斥力が作用せず、成分同士の相対位置は未定義のままです。

成分パッキングは、この「成分間の配置」を担当します。各成分の bounding box を面積降順に並べ、Shelf-packing で左上からコンパクトに詰めることで、画面上に無駄な空白なく配置します。これは2次元パッキング問題の近似解法です。
...
なお、全エンティティがリレーションで繋がっている（連結成分が1つだけの）場合は、この処理は実質スキップされます。
