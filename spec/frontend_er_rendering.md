...
* ホバーインタラクション: パンモード中はホバーイベントを無視する
  ...
  - エンティティ選択状態によるハイライトは表示したまま（影響を受けない）
...
2. `erDiagram.ui.isPanModeActive`が`true`に設定される
...
3. ホバーイベントが発火してもActionがdispatchされず、ハイライト状態が更新されない
4. 矩形・テキストの`onMouseDown`イベントは早期リターンされる

#### パンモード終了時（スペースキー解放 or ホイールボタン解放）

1. カーソルが通常状態に戻る
2. `erDiagram.ui.isPanModeActive`が`false`に設定される
3. エンティティのドラッグが有効化される（`nodesDraggable: true`）
4. 矩形・テキストのドラッグが有効化される
...
* パンモード状態: `erDiagram.ui.isPanModeActive`（boolean）で管理
  - スペースキー押下中またはホイールボタンドラッグ中に`true`
  - 通常時は`false`
* ホバーイベント無効化: `isPanModeActive`を参照して、ホバーActionを早期リターンさせる
...
1. **スペースキー押下チェック**: `effectiveSpacePressed === true`の場合、`e.stopPropagation()`を呼ばずに早期リターン（ドラッグを開始せず、イベントをReact Flowに委譲）
2. **ホイールボタンチェック**: `e.button === 1`の場合、`e.stopPropagation()`を呼ばずに早期リターン（ドラッグを開始せず、イベントをReact Flowに委譲）

**重要**: パンモード中は`e.stopPropagation()`を呼ばないことで、マウスイベントがReact Flowに伝播し、ビューポート移動が正しく実行される。通常モード（矩形・テキストのドラッグを行う場合）のみ`e.stopPropagation()`を呼ぶ。
...
* スペースキー押下の検出: `useKeyPress('Space')`フックで監視する（React Flow提供）
* スペースキー押下時に`isPanModeActive`を`true`に設定するActionをdispatch
* スペースキー解放時に`isPanModeActive`を`false`に設定するActionをdispatch
* ホバーActionの修正: `actionHoverEntity`, `actionHoverEdge`, `actionHoverColumn`の先頭で`isPanModeActive`をチェックし、trueの場合は早期リターン
* `nodesDraggable`の切り替えは`isPanModeActive`の変化時に即座に反映される
...
* **インポート時の互換性**: 古いデータに`isPanModeActive`が存在しない場合は`false`として扱う
...
#### 4. パンモード中・ドラッグ中のホバー無効化

**目的**: パンスクロール中やエンティティドラッグ中に不要なホバーイベントを無視し、描画のちらつきを防ぐ

**無効化される状況**:

1. **パンモード中** (`isPanModeActive === true`):
   - スペースキー押下中またはホイールボタンドラッグ中
   - ホバーイベントが発生してもActionが早期リターンされ、ハイライト状態が更新されない
   - エンティティ選択状態によるハイライトは維持される

2. **エンティティドラッグ中** (`isDraggingEntity === true`):
   - エンティティノードのドラッグ開始時にトリガー
   - 現在のホバー状態とハイライト表示をクリアする
   - ドラッグ中はホバーイベントを無視する
   - ドラッグ終了時に再度有効化される

**視覚効果のtransition**: CSS transitionは使用しない
...
* パンモード中のホバー無効化は、`isPanModeActive`フラグを参照してホバーActionを早期リターンさせる
